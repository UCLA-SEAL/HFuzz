...
event Producer1(queue &q, buffer<int, 1> &input_buffer,buffer<int, 1> &input2_buffer, buffer<int, 1> &size_buf) {
  ...
  auto e = q.submit([&](handler &h) {
    ...
    h.parallel_for<ProducerTutorial>(range(num_elements,num_elements), [=](auto index) {
      ...
      bool flag;
      ProducerToConsumerPipe::write(sum,flag);
      size[0]++;
  ...
  auto e = q.submit([&](handler &h) {
    ...
    h.single_task<ConsumerTutorial>([=]() {
      for (size_t i = 0; i < num_elements; ++i) {
        ...
        bool flag;
        auto input = ProducerToConsumerPipe::read(flag);
        size[0]--;
        int answer = 0;
        // do work on the input
        if (flag){answer = ConsumerWork(input);}
...
int main(int argc, char *argv[]) {
  ...
  std::cout << "\n";
  outfile <<" "<<size_vec[1]<<" "<< (p_end-p_start)*1e-6 <<" "<< (c_end-c_start)*1e-6 <<" "<< total_time_ms <<" "<<throughput_mbs << std::endl;
  // Verify the result
