    ...
    h.parallel_for(num_items, [=](auto i) { sum[i] = a[i] + b[i]; 
                                            if (((sum[i]<0)and(a[i]>=0)and(b[i]>=0)) or ((sum[i]>0)and(a[i]<=0)and(b[i]<=0))){
                                                ...
                                                while (timeout_counter <= kTimeoutCounterMax) {
                                                    MyDeviceToHostSideChannel::write(i,valid_read);
                                                    if (valid_read) {
                                                        sum[0]=sum[0]+1;
                                                        timeout_counter = 0;
                                                        break;
                                                ...
                                                if (timeout_counter>=kTimeoutCounterMax){
                                                      DeviceToHostSideChannel::write(93);
                                                      DeviceToHostSideChannel::write(90);
                                                      DeviceToHostSideChannel::write(75);
                                                      DeviceToHostSideChannel::write(i);
                                                  }
                                            }
                                          }
                  );
