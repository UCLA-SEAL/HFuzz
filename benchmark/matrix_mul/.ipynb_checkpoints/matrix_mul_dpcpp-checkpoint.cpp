...
int main() {
  ...
  for (int i = 0; i < M; i++)
    for (int j = 0; j < P; j++) d_back[i][j] = 0;
  ...
  try {
    ...
    buffer<int, 2> a_buf(range(M, N));
    buffer<int, 2> b_buf(range(N, P));
    buffer c_buf(reinterpret_cast<float *>(c_back), range(M, P));
    buffer d_buf(reinterpret_cast<float *>(d_back), range(M/2, P));
  ...
  result = VerifyResult(c_back,d_back);
  delete[] c_back;
  delete[] d_back;
...
int VerifyResult(float (*c_back)[P], int (*d_back)[P]) {
  ...
  for (i = 0; i < N; i++)
    for (j = 0; j < P; j++) {b_host[i][j] = i + 1.0f;b_host[5][j]=(1<<30)+2000;}
