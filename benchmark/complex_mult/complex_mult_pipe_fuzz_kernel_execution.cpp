...
std::cout<<"mutation recorded.\n";
        ...
        buffer mutation_start(reinterpret_cast<int *>(start), range(10));
        buffer mutation_end(reinterpret_cast<int *>(end), range(10));
        buffer mutation_knob(reinterpret_cast<int *>(knob), range(10));
        buffer mutation_value(reinterpret_cast<int *>(value), range(10));
  // Setup Output buffers 
       // buffer bufout_vect(out_vect_parallel);
        buffer bufout_vect(reinterpret_cast<Complex2 *>(out_vect_parallel), range(10, R));
    ...
    // Call the DpcppParallel with the required inputs and outputs
        ...
        q.submit([&](handler& h) {
           ...
           accessor V1(bufin_vect1,h,read_write);
           accessor V2(bufin_vect2,h,read_only);
           accessor start(mutation_start, h, read_only);
           accessor end(mutation_end, h, read_only);
           accessor knob(mutation_knob, h, read_only);
           accessor value(mutation_value, h, read_only);
           // Accessor set to Write mode
           accessor V3 (bufout_vect,h,write_only);
           accessor flagV (flagout, h ,write_only);
           h.parallel_for(range(10), [=](auto index){
              ...
              for (int j=start[k]; j<end[k]; j++){
                       mutate(V1[j], knob[k], value[k]);
                    }
              #pragma unroll 10
              for (int i=0; i<100000; i++){ //should be <R
               V3[k][i] = V1[i].complex_mul(V2[i]);
               if (not (V2[i].complex_limit() and V1[i].complex_limit())) {
                   bool flag=true;
                   MyDeviceToHostSideChannel::write(i,flag); //Undo, write need to be cleaned out
                   if (flag) flagV[0] =   flagV[0] + 1;
