...
void mutate(Complex2 *a, int knob, int value1, int value2){
    //srand(time(NULL) + rand());
    //int knob = rand()%4+1;
    
    if (knob==1){
        *a=Complex2(value1,value2);
    }
    else if (knob==2){
        *a=Complex2(0,0);
    }
        ...
        *a=Complex2((*a).real()*100000, (*a).imag()*10000);

        //}
    }
    else if (knob==4){
        //for (int i = 1; i < pos_i; i++){
        *a=Complex2((*a).real()/100- 100000, (*a).imag()/100- 100000);
            //if (a[i] < a_min){  
             //   a_min = a[i];
            //}
        //}
...
void DpcppParallel(queue &q, std::vector<Complex2> &in_vect1,
  ...
  q.submit([&](auto &h) {
    ...
    h.parallel_for(R, [=](auto i) {
      ...
      // complex number
      if (i<pos) {mutate(&(V1[i]), knob, value_1, value_2);}
      V3[i] = V1[i].complex_mul(V2[i]); //need to write to different buffer
...
int main(int argc, char* argv[]) {
  ...
  while (i<vector_size){
      input_vect1.push_back(Complex2(rand(), rand()));
      i = i + 1;
  }
  ...
  while (i<vector_size){
      input_vect2.push_back(Complex2(rand(), rand()));
      i = i + 1;
  }
