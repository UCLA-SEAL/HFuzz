...
// SystemVerilog created from k11_ZTS30HostToDeviceDTermSideChannelID_function_cra_agent
...
module k11_ZTS30HostToDeviceDTermSideChannelID_function_cra_agent (
    ...
    // start_bit(BITSELECT,164)
    assign start_bit_b = status_NO_SHIFT_REG_q[0:0];

    // start_or_start_buffered(LOGICAL,169)
    ...
    // start_buffered_and_kernel_idle(LOGICAL,166)
    assign start_buffered_and_kernel_idle_q = not_running_bit_q & buffered_start_NO_SHIFT_REG_q;

    // start_from_buffered_start_NO_SHIFT_REG(REG,167)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // printf_bit(BITSELECT,143)
    assign printf_bit_b = status_NO_SHIFT_REG_q[3:3];

    // printf_bit_mux(MUX,144)
    ...
    // readdata_valid_stage1_reg(REG,151)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // start_bit_computation(LOGICAL,165)
    assign start_bit_computation_q = not_start_q & start_bit_b;

    // status_self_update(BITJOIN,178)
    ...
    // printf_reset_bit(BITSELECT,146)
    assign printf_reset_bit_b = status_NO_SHIFT_REG_q[4:4];

    // acl_counter_reset(GPOUT,121)
    ...
    // padded_finish_counter(BITJOIN,142)
    ...
    // readdata_upper_bits_mux(MUX,150)
    ...
    // avs_cra_readdata(GPOUT,122)
    assign avs_cra_readdata = cra_output_readdata_reg_q;

    // readdata_valid_stage2_reg(REG,152)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // avs_cra_readdatavalid(GPOUT,123)
    ...
    // num_groups_2(GPOUT,137)
    assign num_groups_2 = num_groups_reg_2_q;

    // will_be_started(LOGICAL,181)
    ...
    // start_is_or_going_high(LOGICAL,168)
    ...
    // workgroup_size_NO_SHIFT_REG(REG,184)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
