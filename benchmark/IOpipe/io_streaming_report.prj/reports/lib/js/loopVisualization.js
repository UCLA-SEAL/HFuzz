...
function preProcessNodes() {
    ...
    for (let i = 0; i < bottleneckList.length; i++) {
        ...
        if (limiterStartElement.parent === undefined || limiterEndElement.parent === undefined)
            continue;
...
function isLastInstanceOfLimiterEnd(nodeCopy, limiterObj) {
    ...
    let parentNodeCopy = {};
    // Continue to iterate up the node -> parent -> great parent -> great great parent etc. 
    // until either find last iteration with matching parent ID or get empty parent (aka {})
    while (Object.keys(nodeCopyToRecurse).length !== 0) {
