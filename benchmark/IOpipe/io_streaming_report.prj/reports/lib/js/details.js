...
function getHTMLName(nameText) {
    ...
    let name = nameText;
    if (nameText.indexOf("<") !== -1 || nameText.indexOf(">") !== -1) {
        name = name.replace("<", "&lt;");
        name = name.replace(">", "&gt;");
    }
    return name;
...
function getHTMLDetailsFromJSON(JSON_details, title) {
    if (!JSON_details || !JSON_details.length)
        return "";
    var DETAIL = {
        TEXT: "text",
        BRIEF: "brief",
    ...
    var html_details = (title !== undefined) ? "<h6 class=\"card-title\">" + title.replace(/</g, '&lt;').replace(/>/g, '&gt;') + ":</h6>" : "";
    var list_stack = [];
    ...
    function generateHTMLDetails(html_details, JSON_details, list_stack) {
        JSON_details.filter(function (detail) {
            if (detail.hasOwnProperty('text') && detail['text'].indexOf("Original parent") >= 0)
                return false;
            return true;
        }).forEach(function (detail) {
            if (detail.type !== DETAIL.BRIEF) {
    ...
    function generateSingleHTMLDetail(html_details, JSON_details, list_stack, use_li) {
        ...
        if (JSON_details.type == "table") {
            ...
            Object.keys(JSON_details).forEach(function (key) {
                ...
                if (key.indexOf("Address bit information") != -1) {
