...
void ParallelBitonicSort(int data_gpu[], int n){//, queue &q) {
  ...
  for (int step = 0; step < n; step++) {
    ...
    for (int stage = step; stage >= 0; stage--) {
      ...
      //  if (last_event.has_value()) h.depends_on(last_event.value());
...
void ParallelBitonicSortBuffer(int a[], int n){//, queue &q) {
  // n: the exponent used to set the array size. Array size = power(2, n)
  int size = pow(2, n);
...
//#endif
  ...
  // numbers.
  int *data_usm = (int *)malloc(size * sizeof(int));
