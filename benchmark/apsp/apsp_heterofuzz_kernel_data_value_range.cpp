...
void BlockedFloydWarshallPhase1(queue &q, int *graph, int round) {
  ...
  q.submit([&](handler &h) {
    ...
    int value = rand();
    int pos = rand()%(blocks * block_size);
    int knob = rand()%4+1;
    h.parallel_for<class KernelPhase1>(
        nd_range<1>(blocks * block_size, block_size), [=](nd_item<1> item) {
          ...
          auto i = k + 0.1;
          if (tid<pos) {mutate(&(graph[(round * block_length + k) * nodes + (round * block_length + j)]), knob, value);}


          // Copy data to local memory.
          //logic is wrong. Three kernel executed in order, hard to do in kernel mutation (maybe impossible?)
          ...
          if (block[k][j]<a[0]){a[0]=block[k][j];}
          if (block[k][j]>b[0]){b[0]=block[k][j];}
...
double exec_time=timer_p.Elapsed()-t1;
    ...
    if (i == repetitions) {
      ...
      outfile << amax << std::endl << elapsed_s << std::endl << elapsed_p;
      outfile.close();
    ...
    std::ofstream outfile;
    outfile.open("exec_fpga_info.txt");
    outfile << value_max << std::endl << value_min << std::endl << value_max;
    outfile.close();
