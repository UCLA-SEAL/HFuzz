...
using MyDeviceToHostSideChannel = DeviceToHostSideChannel<DeviceToHostSideChannelID, int, true, 512>;
...
void BlockedFloydWarshallPhase1(queue &q, int *graph, int round) {
  ...
  q.submit([&](handler &h) {
    LocalBlock block(range<3>(block_length, block_length), h);
    ...
    h.parallel_for<class KernelPhase1>(
        nd_range<1>(blocks * block_size, block_size), [=](nd_item<1> item) {
         ...
         // for (int pp=0;pp<10;pp++){
              ...
              graph[block_length * block_length + (round * block_length + k) * nodes +
