...
void BlockedFloydWarshallCompute(const LocalBlock &C,
...
void BlockedFloydWarshallPhase1(queue &q, int *graph, int round) {
  ...
  //      nd_range<1>(blocks * block_size, block_size), [=](nd_item<1> item) {
    for (int tid=0; tid < blocks*block_size; tid++;) {
          ...
          // Compute.
          BlockedFloydWarshallCompute(block, block, block, i, j);
    ...
    //    nd_range<1>(blocks * block_size, block_size), [=](nd_item<1> item) {
      for (int tid=0; tid<blocks*block_size; tid++;)
      {
          ...
          if (tid != round) {
            ...
            BlockedFloydWarshallCompute(off_diag, off_diag, diagonal, i,
                                        j);
  ...
  for (int tid=0; tid<blocks*block_size; tid++;)
      {
          ...
          auto tid = item.get_group(0);
          auto bi = tid / block_count;
          auto bj = tid % block_count;
