...
using MyDeviceToHostSideChannel = DeviceToHostSideChannel<DeviceToHostSideChannelID, double, true, 128>;
...
void mutate(double a, int knob, double value){
  ...
  if (!read.is_open()){
    ...
    // Initializing and generating initial probabilities for the hidden states.
        ...
        for (int k = 0; k<10; k++){
            ...
            outfile << knob[k] <<" "<< start[k] <<" "<<"  "<<end[k]<<" "<<value[k]<<std::endl;
               ...
               buffer<int, 1> start_buf(start,10);
                ...
                buffer<float, 1> value_buf(value,10);
        ...
        for (int k = 0; k<10; k++){
            ...
            q.submit([&](handler& h) {
                auto pi_acc = pi_buf.get_access<access::mode::write>(h);
         ...
         auto k = k_buf.get_access<access::mode::read>(h);
                h.single_task([=]() {
                    for (int j=start[k[0]]; j<end[k[0]]; j++){
                       mutate(pi_acc[j], knob[k[0]], value[k[0]]);
                    }
                });
            });
            cout << "mutation done.\n";
            ...
            q.submit([&](handler& h) {
                ...
                auto pi_acc = pi_buf.get_access<access::mode::read_write>(h);
	  ...
	  #pragma unroll 4 
	     ...
	     for (int j=0; j<T; j++){
                ...
                v_acc[i][j] = (j != 0) ? MIN_DOUBLE : pi_acc[i] + b_acc[i][seq_acc[0]];
                if (v_acc[i][j]<a[0]){a[0]=v_acc[i][j];}
                if (v_acc[i][j]>b[0]){b[0]=v_acc[i][j];}
                bool flag=true;
