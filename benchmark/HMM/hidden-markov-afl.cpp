  ...
  if (!read.is_open()){
    ...
    // Initializing and generating initial probabilities for the hidden states.
        ...
        //q.submit([&](handler& h) {
            ...
            for(int i=0; i<N; i++)
              for (int j=0; j<M; j++)
                ...
                double prob = ((i + j) % M) * 2.0f / M / (M - 1);
                // The algorithm computes logarithms of the probability values to improve small numbers processing.
                b[i][j] = (prob == 0.0f) ? MIN_DOUBLE : a[i][j];
              }
            //});
        //});
        ...
        //q.submit([&](handler& h) {
            ...
            for(int i=0; i<N; i++)
              for (int j=0; j<T; j++)
                ...
                viterbi[i][j] = (j != 0) ? MIN_DOUBLE : pi[i] + b[i][seq[0]];
        ...
        for (int i = T - 2; i >= 0; --i) {
            ...
            cout << "The Viterbi path is: "<<vit_path[i + 1]<< std::endl;
            if (vit_path[i+1]<0) vit_path[i+1]=i;
