        ...
        function parseJson(loopBlock, pParentId, json) {
            ...
            if (json && json.hasOwnProperty('details')) {
                for (let i = 0; i < json['details'].length; ++i) {
                    let detail = json['details'][i];
                    if (detail.hasOwnProperty('type') && detail.hasOwnProperty('text'))
                        if (detail['type'] === "brief" && detail['text'] !== "") {
                            briefStr += detail['text'];
                            briefStr += "; ";
                        }
                }
                if (briefStr.length >= 2) {
                    briefStr = briefStr.slice(0, briefStr.length - 2);
                }
            }
            columns['brief'] = briefStr;
            ...
            if (jsonHasChildrenArray(loopBlock)) {
                loopBlock['children'].forEach(function (child) {
                    if (child['type'] === 'loop') {
                        let vSubLoopName = getRealName(child['name']);
