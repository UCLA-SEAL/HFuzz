    ...
    if (this.vTranspose) {
            ...
            if (_this.vAttrColumnMap) { // no data when no attribute map
                _this.vDataList.forEach(function (row) {
                    ...
                    if (_this.vEventColumns.indexOf(_this.vAttrColumnMap[col]) !== -1)
                        _this._newCell(vTmpRow, 'td', vValue, col.replace(/\s+/g, '') + "Col", _this.vDataWidth, row.getClass(), row.getTitle(), row.getNote());
                    else
                        _this._newCell(vTmpRow, 'td', vValue, col.replace(/\s+/g, '') + "Col", _this.vDataWidth, row.getClass());
                });
            }
        ...
        for (let i = 0; i < this.vDataList.length; i++) {
            ...
            if (this.vShowName) {
                let vNameCell;
                if (_this.vEventRow)
                    vNameCell = this._newCell(vTmpRow, 'td', "<xmp>" + row.getName().replace(/&lt;/g, '<').replace(/&gt;/g, '>') + "</xmp>", '', this.vNameWidth, '', row.getTitle());
                else if (!_this.vEventRow && _this.vEventColumns.indexOf('name') !== -1)
                    vNameCell = this._newCell(vTmpRow, 'td', "<xmp>" + row.getName().replace(/&lt;/g, '<').replace(/&gt;/g, '>') + "</xmp>", '', this.vNameWidth, '', row.getTitle(), row.getNote());
                else
                    vNameCell = this._newCell(vTmpRow, 'td', "<xmp>" + row.getName().replace(/&lt;/g, '<').replace(/&gt;/g, '>') + "</xmp>", '', this.vNameWidth, '', '', row.getNote());
 ...
 * @param {string} pID is the id of the cell
 ...
 */
FPGADataTable.prototype._newCell = function (pParent, pType, pText, pID, pWidth, pClass, pTitle, pCallback) {
    if (pType !== 'th' && pType !== 'td')
        return;
    var vTitle = (pTitle !== undefined) ? pTitle : null;
    var vCallback = (pCallback !== undefined) ? pCallback : null;
    var vNewCell = newFPGAElement(pParent, pType, pClass, pID, pText);
    if (pWidth)
