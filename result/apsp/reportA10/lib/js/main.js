...
/* jshint -W097 */
/// Global variables
// JSON objects that contain the report data
var infoJSON, quartusJSON, areaJSON, area_srcJSON, mavJSON, gmvJSON, lmvJSON, loopsJSON, loop_attrJSON, summaryJSON, warningsJSON, fileJSON, pipelineJSON, treeJSON, new_lmvJSON, systemJSON, blockJSON, scheduleJSON, bottleneckJSON;
var alpha_viewer;
...
var MENU = {
    ...
    THROUGHPUT: { value: 2, name: 'Throughput Analysis', submenu: ['LOOP', 'divider', 'VERIF'] },
...
/**
  * @function preprocessJSON  does a few things:
  * 1. search and replaces all instances of < and > in strings in various global JSON objects.
  * 2. processes area json to reduce duplicate source locations.
  */
function preprocessJSON() {
    //1. sanitize json strings
    sanitizeHelper(infoJSON);
    sanitizeHelper(quartusJSON);
    sanitizeHelper(areaJSON);
    sanitizeHelper(area_srcJSON);
    sanitizeHelper(loopsJSON);
    sanitizeHelper(loop_attrJSON);
    sanitizeHelper(summaryJSON);
    sanitizeHelper(warningsJSON);
    sanitizeHelper(pipelineJSON);
    sanitizeHelper(treeJSON);
    sanitizeHelper(new_lmvJSON, true);
    sanitizeHelper(systemJSON);
    sanitizeHelper(blockJSON);
    sanitizeHelper(scheduleJSON);
    sanitizeHelper(bottleneckJSON);
    //2. Area JSON Processing
    try {
        let areaProcessor = new AreaProcessor(areaJSON);
        let reducedAreaJSONProcessor = areaProcessor.reduceAreaJSON();
        areaJSON = reducedAreaJSONProcessor.getJSON();
    }
    catch (err) {
        console.error(`Unable to reduce areaJSON. ${err}`);
    }
}
/*
 * @function sanitizeHelper DFS all strings fields in an object and replces < and > html entities with proper escape
 */
function sanitizeHelper(obj, searchForDblPump = false) {
    for (const property in obj) {
        if (typeof (obj[property]) == "object") {
            sanitizeHelper(obj[property], searchForDblPump);
        }
        else if (typeof (obj[property]) == "string") {
            if (searchForDblPump && property === "Pump configuration" && obj[property] === "Double-pumped") {
                hasDoublePumpedMem = true;
            }
...
function loadReportData() {
    return __awaiter(this, void 0, void 0, function* () {
        if (typeof window.api !== 'undefined') {
            let projectData = yield window.api.retrieveProjectData();
            infoJSON = projectData['info'];
            quartusJSON = projectData['quartus'];
            areaJSON = projectData['area'];
            area_srcJSON = projectData['area_src'];
            mavJSON = projectData['mav'];
            gmvJSON = projectData['gmv'];
            lmvJSON = projectData['lmv'];
            loopsJSON = projectData['loops'];
            loop_attrJSON = projectData['loop_attr'];
            summaryJSON = projectData['summary'];
            warningsJSON = projectData['warnings'];
            fileJSON = projectData['file'];
            pipelineJSON = projectData['pipeline'];
            treeJSON = projectData['tree'];
            new_lmvJSON = projectData['new_lmv'];
            systemJSON = projectData['system'];
            blockJSON = projectData['block'];
            scheduleJSON = projectData['schedule'];
            bottleneckJSON = projectData['bottleneck'];
            alpha_viewer = false;
        }
    });
}
    ...
    function addReportColumn(reportEnum) {
        ...
        switch (reportEnum) {
            ...
            case VIEWS.LOOPVIS:
                ...
                return;
            default:
                console.error('Invalid report column view: ' + reportEnum.value + ' (' + reportEnum.name + ')');
                return;
        }
        report += '</div></div></div>';
...
function goToView(viewId, update) {
    ...
    switch (view) {
        ...
        case VIEWS.GV:
            ...
            systemTree.generateTests(false);
            vBottleneckTree.draw('');
            break;
        case VIEWS.SCHEDULE:
            systemTree = new FPGASystemViewerTree(document.getElementById('tree-list'), title, systemNodeName);
        ...
        case VIEWS.AREA_SRC:
            ...
            break;
        default:
            console.error('Invalid tree view: ' + view.value + ' (' + view.name + ')');
            break;
    }
...
$(document).ready(function () {
    ...
    $('#report-pane').on('click', '.parent', toggleRows);
    ...
    $(function () {
        $('[data-toggle="tooltip"]').tooltip({
            trigger: 'hover',
            placement: function () {
                if ($(".navbar-toggler").is(":visible")) { // if the hamburger menu is enabled
                    return "bottom";
                }
                return "right";
            }
