...
// ************* schdule viewer (beta) ***************************
function generateScheduleTests(dir, view, category, namePrefix) {
    if (!vScheduleGantt)
        return;
    const nodeFunctions = {
        //returns list of JSGantt.TaskItem's
        getNodes: function () {
            return vScheduleGantt.getList();
        },
        clickOnNode: function (node) {
            node.getBarDiv().click();
        },
        getName: function (node) {
            return $(node.getNotes()).find('.card-title').text();
        }
    };
    const testData = {
        dir: dir + 'gantNodes/',
...
function flattenObjBeta(node, nodeIDSrcDict, parentId, g) {
    ...
    if (node.hasOwnProperty("children")) {
        ...
        let sortedChildren = topologicalNodeSort(children, nodeIDSrcDict);
        sortedChildren.forEach(function (childObj) {
 ...
 * @param {*} nodeIDSrcDict Maps the nodes's id to the ids of its children
 * @returns A topologically sorted array of the nodes in <nodes>
 */
function topologicalNodeSort(nodes, nodeIDSrcDict) {
    let idIndexMap = new Map();
    let treeMap = new Map();
    for (let i = 0; i < nodes.length; i++) {
        idIndexMap.set(nodes[i].id, i);
    }
    for (let i = 0; i < nodes.length; i++) {
        if (nodeIDSrcDict.hasOwnProperty(nodes[i].id)) {
            let childrenIDs = nodeIDSrcDict[nodes[i].id].toString().split(',');
            let childrenNodes = [];
            childrenIDs.forEach((childID) => {
                childrenNodes.push(nodes[idIndexMap.get(Number(childID))]);
            });
            treeMap.set(i, childrenNodes);
        }
        else {
            treeMap.set(i, []);
        }
    ...
    let hashMap = new Map();
    let isRoot = Array(nodes.length).fill(true);
    for (let i = 0; i < nodes.length; i++) {
        generateSubtreeHash(nodes, idIndexMap, treeMap, hashMap, isRoot, i);
    }
    let rootNodeIndex = [];
    for (let i = 0; i < nodes.length; i++) {
        if (isRoot[i])
    ...
    rootNodeIndex.sort((index1, index2) => {
        return hashMap.get(index1) - hashMap.get(index2);
    });
    let sortedArray = [];
    let inSortedArray = Array(nodes.length).fill(false);
    for (let i = 0; i < rootNodeIndex.length; i++) {
        generateSortedArray(nodes, sortedArray, inSortedArray, idIndexMap, treeMap, rootNodeIndex[i]);
    }
    return sortedArray;
}
/**
 * Generate the hash code for the node <nodes[nodeIndex]> and its children in the tree.
 * @param {*} nodes An array of nodes
 * @param {*} idIndexMap Map the node's id to its index in <nodes>
 * @param {*} treeMap Map the node's index in <nodes> to its children
 * @param {*} hashMap Map the node's index in <nodes> to its hashcode
 * @param {*} isRoot Map the node's index to a boolean value that indicates whether a node is the root of the tree.
 * @param {*} nodeIndex The index of the node to generate the hashcode for
 * @returns
 */
function generateSubtreeHash(nodes, idIndexMap, treeMap, hashMap, isRoot, nodeIndex) {
    if (hashMap.get(nodeIndex) !== undefined) {
        return;
    }
    let node = nodes[nodeIndex];
    let nameHashCode = node.name.hashCode();
    let children = treeMap.get(nodeIndex);
    children.forEach((child) => {
        isRoot[idIndexMap[child.id]] = false;
        generateSubtreeHash(nodes, idIndexMap, treeMap, hashMap, isRoot, idIndexMap.get(child.id));
    });
    children.sort((element1, element2) => {
        return hashMap.get(idIndexMap.get(element1.id)) - hashMap.get(idIndexMap.get(element2.id));
    });
    let hashCodeString = nameHashCode.toString();
    for (let i = 0; i < children.length; i++) {
        hashCodeString += hashMap.get(idIndexMap.get(children[i].id)).toString();
    }
    let nodeHashCode = hashCodeString.hashCode();
    hashMap.set(nodeIndex, nodeHashCode);
}
/**
 * Generate a sorted array based on the postorder traversal of the tree.
 * @param {*} nodes An array of nodes
 * @param {*} sortedArray An array sorted by on the postorder traversal of the tree
 * @param {*} inSortedArray Map the node's index in <nodes> to a boolean value that indicates if the node is in sortedArray
 * @param {*} idIndexMap Map the id of a node to its index in <nodes>
 * @param {*} treeMap Map a node's index in <nodes> to its children
 * @param {*} nodeIndex Tne node with <nodeIndex> and its children are inserted into the sorted array next.
 * @returns
 */
function generateSortedArray(nodes, sortedArray, inSortedArray, idIndexMap, treeMap, nodeIndex) {
    if (inSortedArray[nodeIndex])
        return;
    let children = treeMap.get(nodeIndex);
    for (let i = 0; i < children.length; i++) {
        generateSortedArray(nodes, sortedArray, inSortedArray, idIndexMap, treeMap, idIndexMap.get(children[i].id));
    }
    sortedArray.push(nodes[nodeIndex]);
    inSortedArray[nodeIndex] = true;
}
