...
let FPGAGraph = function (jsonData, rankdir, isBankView, jsonBottleneck) {
    ...
    this.getNodeOptions = function (n) {
        ...
        shareArbColor = '#005A9D', // oneAPI medium blue
        ...
        switch (n['type']) {
            ...
            case 'arb':
                ...
                if (n.hasOwnProperty('name') && n['name'] === 'SHARE') {
                    nodeOptions['style'] = 'fill: ' + shareArbColor + ';';
                    // Change text to white to pass color contrast accessibility (ratio 7.12:1)
                    nodeOptions['labelStyle'] = 'fill: ' + defaultColor + ';';
                }
                else {
                    ...
                    if (n['name'] === 'Load' || n['name'] === 'Store' ||
                        ...
                        if (n['details'][0]['Global Memory'] && n['details'][0]['Global Memory'] == 'Yes') {
                            ...
                            for (let ei = 0; ei < this.allEdges.length; ei++) {
                                ...
                                if ((e['from'] === n['id'] && this.flattenedNodes.get(e['to']) !== undefined &&
                                    this.flattenedNodes.get(e['to'])['parent'] !== undefined &&
                                    this.flattenedNodes.get(e['to'])['parent']['name'] === 'Global Memory') ||
                                    (e['to'] === n['id'] && this.flattenedNodes.get(e['from']) !== undefined &&
                                        this.flattenedNodes.get(e['from'])['parent'] !== undefined &&
    ...
    this.generateTests = function (dir, view, category, namePrefix) {
        const nodeFunctions = {
            getNodes: function () {
                let nodes = [];
                let selection = d3.select("svg g").selectAll("g.node rect, g.cluster rect, g.node ellipse, g.node polygon");
            ...
            getName: function (node) {
                return d3.select(node.parentNode).text();
            },
...
let FPGAGVGraph = function (pNode) {
    ...
    this.draw = function (pDiv) {
        ...
        // The "component" object, which displays the basic blocks tree, is extracted from mav.json.
        // This file is generated in Analysis_Visualization.cpp in the compiler, and operates directly
        // on the XNodes themselves, rather than the Abstract Netlist like the other passes. As a result,
        // the IDs generated in mav.json are unique, and different from those used everywhere else. Since
        // the bottleneck IDs are generated from the Abstract Netlist, their data is meaningless to the
        // component tree.
        let bneck = bottleneckJSON;
        if (pNode.type === "component")
            bneck = undefined;
        // Create FPGAGraph and render it
        let gv_graph = new FPGAGraph(this.source, this.rankdir, false, bneck);
    ...
    this.generateTests = function (dir, view, category, namePrefix) {
        return (this.graph) ? this.graph.generateTests(dir, view, category, namePrefix) : null;
    };
};
/**
