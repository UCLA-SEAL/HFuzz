...
static std::vector<queue_entry*> input_queue_1; /* First Fuzzing queue for first IG*/
static std::vector<queue_entry*> input_queue_2; /* Second Fuzzing queue for second IG */
...
static bool devcloud_fpga_hd_enable = 1; /*enable devcloud fpga*/
static bool devcloud_gpu_enable = 0;  /*enable devcloud gpu*/
static bool queue_choose = true;
...
static void list_dir(const char *path)
{
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        if (queue_choose) {
            input_queue_1.push_back(q);   
        }
        else {
          input_queue_2.push_back(q);    
        }
        queue_choose = !queue_choose;
    }
...
void fuzzing(char* app, int iteration){
  ...
  for(int i = 1; i < iteration; i ++){
    ...
    srand(time(0) + rand());
    if (queue_choose) {
        total_index = input_queue_1.size();
    }
    else {
        total_index = input_queue_2.size(); 
    }  
  
    int index = rand()%total_index;
        printf("input queue length: %d\n", total_index);
     for(int i = 0; i < total_index; i++){
       if (queue_choose) {printf("%s\n ", input_queue_1[i]->fname);}
       else {printf("%s\n ", input_queue_2[i]->fname);}
     }
    
    struct queue_entry* q;
    if (queue_choose) {q = input_queue_1[index];}
    else {q = input_queue_2[index];}
...
void perform_dry_run(char* app){
  ...
  char* argv[] = {app, "~/Heterofuzz/prototype/good-seeds/seed.txt", NULL};
  ...
  input_queue_1[0]->exec_cksum = ck1;
  
  input_queue_1[0]->has_new_cov = 1;
...
int main(int argc, char** argv) {
  ...
  list_dir(in_dir);
  OKF("Input queue initialized with %d seeds.", input_queue_1.size());
  // for(int i = 0; i < input_queue_1.size(); i++){
  //   printf("%s\n", input_queue_1[i]->fname);
  // }
